from PyQt5.QtWidgets import QApplication, QMainWindow, QTextEdit, QStackedWidget, QWidget, QLineEdit, QGridLayout, QVBoxLayout, QHBoxLayout, QPushButton, QFrame, QLabel, QSizePolicy, QSpacerItem
...
class ChatSection(QWidget):
     ...
     def loadMessages(self):
          ...
          with open(TempDirectoryPath('Response.data'), "r", encoding='utf-8') as file:
               ...
               if messages and messages.strip() and messages != old_chat_message:
                    self.add_message_signal.emit(messages, 'White')  # emit signal
     ...
     def _addMessage(self, message, color):
          try:
               self.chat_text_edit.setTextColor(QColor(color))
...
class InitialScreen(QWidget):
     def __init__(self, parent=None):
          ...
          gif_width = int(screen_width * 0.6)
          max_gif_size_H = int(gif_width * 11 / 15)
          movie.setScaledSize(QSize(gif_width, max_gif_size_H))
          ...
          # self.toggled = True
          # self.toggle_icon()  # Initial icon state

          # ✅ Spacer to prevent crowding (but no forced black block)
...
def GraphicalUserInterface():
     from Main import MainExecution  # ✅ avoid circular import
